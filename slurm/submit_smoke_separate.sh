...
#
#   sbatch slurm/submit_smoke_separate.sh [run_name]
#
...
RUN_NAME="${1:-smoke_sep_$(date +%Y%m%d_%H%M%S)}"
...
python -u src/switch_pipeline_separate.py \
    --config configs/pdl1_pcna_separate.yaml \
    --run-name "$RUN_NAME" \
    --smoke \
